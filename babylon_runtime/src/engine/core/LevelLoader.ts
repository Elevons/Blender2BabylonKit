...
export class LevelLoader
{
  ...
  async Load(manifestUrl: string): Promise<Level>
  {
    ...
    // Shadows, scene-wide render settings, and animations.
    ...
    {
      this.ApplyScene(manifest.scene, baseUrl, context.level);
    }
    ...
    if (this.options.debugColliders)
    {
  ...
  private ProcessEntity(entityData: EntityData, context: LoadContext): void
  {
    ...
    context.pendingReferences.push(...this.ApplyComponents(entity, entityData.components));
   ...
   * RIGIDBODY, set the tag, and instantiate SCRIPT behaviors (deferring their
   * entity references). Returns the deferred references for the second pass.
   */
