...
}
{
  ...
  let camera: Camera;
  let followTarget: CameraTargetBinding | undefined;
  let offsetFollow: CameraTargetBinding | undefined;
  let arcTarget: CameraTargetBinding | undefined;
  ...
  else if (cameraComponent.cameraType === "ARC")
  {
    ...
    {
      arcTarget = { guid: cameraComponent.target, eye: eyePosition };
    }
  }
  else if (cameraComponent.followMode === "OFFSET")
  {
    ...
    if (cameraComponent.target !== null)
    {
      offsetFollow = { guid: cameraComponent.target, eye: eyePosition };
    }
  }
  else
  {
    // FOLLOW / ORBIT: Babylon FollowCamera (offset rotates with the target's yaw).
    ...
    if (cameraComponent.target !== null)
    {
      followTarget = { guid: cameraComponent.target, eye: eyePosition };
    }
  }

  CopyLens(sourceCamera, camera);
