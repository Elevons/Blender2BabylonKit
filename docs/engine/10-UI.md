...
Engine code lives under `packages/engine/src/ui/` (2D + 3D GUI) and
...
Runtime: `ui/gui2d.ts` → `ApplyGui` → `entity.guiTextures`. Lookup:
...
1. `SettleTasks` for audio, GUI, and particle promises (`allSettled`)
2. `WireTriggerEvents`
3. `BuildConstraints`
4. `BuildGui3DControls` → `level.gui3DManager`
5. `level.Begin()`
...
| Concern | Module |
|---|---|
| Component types + fields | `properties.py` (`GUI3D_CONTROLS` / `GUI3D_PANELS` sets) |
| Panel + click-event UI | `ui.py` (`_draw_click_events`) |
| Click row operators | `operators.py` (`bjs.gui3d_event_add/remove`) |
| Serialize + copy assets | `export.py` (`_serialize_components`, `_copy_asset`) |
| Validation | `validate.py` (`_check_media`, `_check_gui3d`) |
