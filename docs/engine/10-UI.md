...
[Babylon's Particle Editor](https://particles.babylonjs.com) or the Node
Particle Editor (`.json` with `BABYLON.NodeParticleSystemSet`).
...
| **Particle Textures** (list) | Optional image overrides (see below) |
| **Use GPU** / **Auto Start** / **Attach to Object** / **Max Particles** | Runtime options (see manifest) |

Each **Particle Textures** row:
...
| **Replace URL** | Only patch blocks whose current URL matches. Empty → auto (first empty block, or the only block) |

Export calls `export/particles.py` → `export_particle_system`: copies the JSON,
copies texture images, then patches `ParticleTextureSourceBlock.url` in the
**exported** copy (the manifest still only references `particles/<stem>.json`).

### Runtime load

`subsystems/particles.ts` → `LoadParticleSystems` (legacy `ParticleSystem` / GPU,
or `NodeParticleSystemSet.buildAsync`). Before build, `ResolveNodeParticleSetTextureUrls`
rewrites bare filenames in the JSON to absolute URLs beside the file (`rootUrl`).
Options: GPU when supported, `autoStart`, `attachToEntity`, optional `capacity`
override.
...
Already-spawned particles still simulate in **world space** unless the particle
file sets `isLocal: true` (local / emitter space in the Particle Editor).
...
2. `WireParticleEmitterTracking` → `level.particleEmitterManager` (when any
   empty-node emitters exist)
3. `WireMsdfTextRendering` → `level.msdfTextManager` (when any labels exist)
4. `WireTriggerEvents`
5. `BuildConstraints`
6. `BuildGui3DControls` → `level.gui3DManager`
7. `level.Begin()`
...
| Particle texture UI | `ui/component_draw.py` (`_draw_particle_textures`) |
| Texture row operators | `operators/components.py` (`bjs.particle_texture_add/remove`) |
...
| Serialize + copy assets | `export/components.py` + `export/particles.py` (`export_particle_system`) + `export/assets.py` |
