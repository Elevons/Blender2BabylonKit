  ...
  },
  {
    ...
    steps: [
      ...
      { file: "packages/engine/src/subsystems/constraints.ts", symbol: "BuildConstraints",
        note: "FINALIZE — both bodies exist now. Per registration: resolve target, require body on both ends, compute the frame, create, addConstraint, set isCollisionsEnabled from manifest collision (all types), optional motor. Out: level.constraints." },
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "blender_addon/export/scene.py", symbol: "serialize_scene",
        note: "BLENDER — assembles the manifest scene block: clear/ambient, environment (World Output chain via scene/environment.py → env/, or useDefault; createSkybox; skyboxIgnoreFog), fog, post (via export/post_processing.py), inputActions, and defaultInputMap." },
