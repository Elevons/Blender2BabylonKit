...
export const TRACES = [
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "packages/engine/src/core/loader/entityBuilder.ts", symbol: "ApplyComponents",
        note: "LOAD — trigger colliders with events queue a TriggerRegistration {sourceEntity, events}; events from multiple trigger colliders on one entity are merged; wiring waits for FinalizeLevel (the plugin observable needs physics live)." },
  ...
  },
  {
    ...
    steps: [
      { file: "blender_addon/export/particles.py", symbol: "export_particle_system",
        note: "EXPORT — copy particle JSON to particles/; copy each Particle Textures image; patch_particle_json_textures (URL in JSON / Replace URL) on the exported file." },
      { file: "packages/engine/src/core/loader/entityBuilder.ts", symbol: "ApplyComponents",
