...
export const TRACES = [
  {
    ...
    steps: [
      ...
      { title: "The manifest (data between the two halves)",
        ...
        note: "Already Babylon-space (Y-up). Two COLLIDER rows on one entity are intentional — the runtime combines them. makeInvisible hides the mesh at load; physics is unaffected." },
      ...
      { file: "packages/engine/src/subsystems/physics/buildPhysics.ts", symbol: "BuildPhysics",
      ...
      { file: "packages/engine/src/subsystems/physics/bodyBuild.ts", symbol: "BuildCompoundBody",
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "packages/engine/src/core/loader/entityBuilder.ts", symbol: "ResolveObjectReferences",
        note: "SECOND PASS — every entity exists; each PendingRef's GUID resolves via level.ById and the real Entity is assigned (or placed into its list slot). Then Begin → OnStart." },
    ],
  },
  {
    id: "lifecycle",
    title: "Behavior lifecycle: hooks → Babylon render loop",
    intro: "How OnStart / OnPostReady / OnUpdate / OnDestroy / OnMessage plug into Level.Begin, late rendering, scene.onBeforeRenderObservable, and Entity.SendMessage — from class definition through teardown.",
    steps: [
      { file: "packages/engine/src/scripting/Behavior.ts", symbol: "Behavior",
        note: "THE contract — overridable hooks (PascalCase only; lowercase names never run). OnPostReady for level.post after late rendering. Injected before OnStart: entity, scene; node getter; optional behavior.input." },
      { file: "packages/engine/src/core/loader/scripts.ts", symbol: "InstantiateScripts",
      ...
      { file: "packages/engine/src/core/loader/entityBuilder.ts", symbol: "ResolveObjectReferences",
        note: "SECOND PASS — @exposed entity refs resolve to real Entity instances (or list slots) before hooks run. Cross-entity OnStart order is unspecified after this." },
      { file: "packages/engine/src/core/loader/finalizeLevel.ts", symbol: "FinalizeLevel",
        note: "Async tasks settle → triggers/constraints/GUI3D wire → level.Begin() → ApplyLateRendering (NME + post on active camera) → NotifyPostReady(). OnStart never runs before physics bodies and trigger observers exist." },
      { file: "packages/engine/src/core/Level.ts", symbol: "Begin",
        note: "InputManager.Attach (keyboard observable + enable maps) → every behavior.OnStart (error-isolated) → subscribe RunFrame on scene.onBeforeRenderObservable. Post and OnPostReady run after this returns." },
      { file: "packages/engine/src/core/Level.ts", symbol: "NotifyPostReady",
        note: "Once per level load — sets level.postReady and runs every behavior.OnPostReady (error-isolated). Spawn/runtime SCRIPT adds call RunPostReady after OnStart when postReady is already true." },
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "packages/engine/src/core/attachments.ts", symbol: "RegisterAttachment",
        note: "The single write path — one EntityAttachment row per successfully applied component: { type, data } plus runtime handle when one exists (behavior, body, sound, texture, system, constraint, control). Mirrors the matching convenience field (entity.tag, entity.body, entity.behaviors, …)." },
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "packages/engine/src/subsystems/collisions/wiring.ts", symbol: "WireCollisionEvents",
        note: "Plugin observers on collision + trigger observables. Relays hooks to both bodies; dispatches Event Messages by phase." },
      { file: "packages/engine/src/subsystems/collisions/dispatch.ts", symbol: "DeliverEventMessages",
      ...
      { file: "packages/engine/src/scripting/Behavior.ts", symbol: "OnMessage",
        note: "RECEIVE Event Messages. OnCollisionEnter/Stay/Exit and OnTriggerEnter/Exit for programmatic hooks." },
    ],
  },
  {
    id: "constraint",
    ...
    steps: [
      ...
      { file: "packages/engine/src/subsystems/constraints.ts", symbol: "BuildConstraints",
        note: "FINALIZE — both bodies exist. Per registration: resolve target, require body on both ends, compute the frame, create, addConstraint, RegisterAttachment on owner, set isCollisionsEnabled from manifest collision (all types), optional motor. Out: level.constraints." },
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "packages/engine/src/core/loader/finalizeLevel.ts", symbol: "FinalizeLevel",
        note: "Shadows → scene look (env, fog, atmosphere when enabled) → animations → settle audio/GUI/particle/MSDF tasks → WireParticleEmitterTracking + WireMsdfTextRendering → trigger wiring → constraints → BuildGui3DControls → Begin → ApplyLateRendering → NotifyPostReady → (debug colliders if the export allows)." },
      { file: "packages/engine/src/core/Level.ts", symbol: "Begin",
        note: "Attach InputManager (enables every action map), run every OnStart (error-isolated), subscribe RunFrame." },
      { file: "packages/engine/src/core/Level.ts", symbol: "NotifyPostReady",
        note: "Run every OnPostReady once after post attach; sets level.postReady." },
      { file: "packages/engine/src/core/Level.ts", symbol: "RunFrame",
  ...
  },
  {
    ...
    intro: "Unity Input System style: the Blender Input Actions panel authors the scene asset (maps > actions > bindings, labeled gamepad pickers, Stick vs 1D/2D composites, axisHalf on 1D axis parts) plus a Scene Default map; the manifest carries inputActions + defaultInputMap; InputManager evaluates every frame (most-actuated binding wins).",
    steps: [
      { file: "blender_addon/input_actions/serialize.py", symbol: "serialize_input_asset",
        note: "BLENDER — maps/actions/bindings → scene.inputActions (axisHalf POSITIVE/NEGATIVE on composite axis parts, LT/RT as gamepad axis 4/5). Built-in Player when the panel is empty." },
      { file: "blender_addon/export/scene.py", symbol: "serialize_scene",
        note: "BLENDER — assembles the manifest scene block: clear/ambient, environment (World Output chain via scene/environment.py → env/, or useDefault with intensity/rotationY from bjs_scene; createSkybox forced off when Atmosphere on; skyboxIgnoreFog), fog, atmosphere (export/atmosphere.py), post (via export/post_processing.py — default pipeline, SSAO), inputActions, defaultInputMap, clusterPunctualLights, lightBudget." },
      ...
      { file: "packages/engine/src/input/InputAction.ts", symbol: "Process",
        note: "Per action: resolve bindings (most-actuated whole binding wins — no per-axis merge), advance the BUTTON/VALUE/PASSTHROUGH phase machine, set edge flags for polling." },
      { file: "packages/engine/src/input/Devices.ts", symbol: "OrientGamepadAxis",
        note: "Stick Y axes (indices 1, 3): browser reports −1 up; flip so authored + Half on Left Stick Y means push up." },
      { file: "packages/engine/src/input/InputBinding.ts", symbol: "ApplyAxisHalf",
        note: "After Y-orient: axisHalf POSITIVE/NEGATIVE reads one stick direction on composite axis parts. Triggers: axis 4/5 → W3C buttons 6/7 as 0..1." },
      { file: "packages/engine/src/input/InputBinding.ts", symbol: "ResolveBinding",
        note: "Direct key/button/axis/stick or 1DAXIS/2DVECTOR composite. Prefer stick binding for 2D gamepad; axis half only on 1D composite axis rows." },
      { file: "game/src/behaviors/InputMover.ts", symbol: "OnUpdate",
      ...
      { file: "game/src/behaviors/CarController.ts", symbol: "OnUpdate",
        note: "CONSUMER — @inputMap(\"Vehicle\"); ReadVector2() on Main Control (y=throttle, x=steer). Author: Value + Vector 2, WASD 2D Vector + Left Stick." },
    ],
  },
  {
    id: "livelink",
    ...
    steps: [
      ...
      { file: "game/vite.config.ts", symbol: "ReloadOnLevelExport",
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "blender_addon/materials/detail_pack.py", symbol: "pack_detail_map",
        note: "EXPORT — pack separate PNG/JPG/WEBP channels into Babylon Unity-layout PNG via bpy.data.images." },
      { file: "blender_addon/export/detail_maps.py", symbol: "serialize_detail_maps",
        note: "EXPORT — copy packed detail texture or pack separate channels; write manifest detailMaps[] (coordinatesIndex, uvScale, blend levels, normalBlendMethod)." },
      { file: "blender_addon/export/level.py", symbol: "_build_manifest",
        note: "serialize_materials adds optional top-level manifest.materials[] (name, file, textures[], inputs[], gradients[]) for exportable meshes using a Material with bjs_nme_file set. serialize_detail_maps adds optional detailMaps[] (coordinatesIndex, uvScale, blend levels)." },
      { file: "packages/engine/src/core/LevelLoader.ts", symbol: "Load",
        note: "LOAD — after appendSceneAsync + ApplyNodeVisibility, await ApplyNodeMaterials then ApplyDetailMaps(scene, manifest.detailMaps, baseUrl) before the entity pass." },
      { file: "packages/engine/src/subsystems/materials/apply.ts", symbol: "ApplyNodeMaterials",
      ...
      { file: "packages/engine/src/subsystems/materials/detailMapApply.ts", symbol: "ApplyDetailMaps",
        note: "After NME replacement: match scene.materials by name, set material.detailMap (texture, coordinatesIndex, uvScale, blend levels). Skips NodeMaterials; marks material dirty." },
      { file: "packages/engine/src/core/loader/finalizeLevel.ts", symbol: "FinalizeLevel",
      ...
      { file: "packages/engine/src/subsystems/materials/apply.ts", symbol: "BuildNodeMaterials",
  ...
  },
  {
    ...
    steps: [
      { file: "blender_addon/export/datablocks.py", symbol: "serialize_light",
        note: "EXPORT — per lamp object: color, intensity, sunAngle (SUN) / spot cone (SPOT), cluster: false when Cluster When Over Budget is off (point/spot), shadow block when Cast Shadows is on. Written into entities[].light on the manifest." },
      { file: "packages/engine/src/core/loader/entityBuilder.ts", symbol: "ProcessEntity",
      ...
      { file: "packages/engine/src/subsystems/lights.ts", symbol: "FindLightForNode",
        note: "Walks the light parent chain (orientation-correction node may sit between GUID node and Light). Searches lights moved into ClusteredLightContainer — required for runtime intensity scripts after clustering." },
      ...
      { file: "packages/engine/src/subsystems/clusteredLights.ts", symbol: "ClusterPunctualLightsIfNeeded",
        note: "FINALIZE (before SetupShadows) — when enabled scene lights exceed lightBudget (default 8): move eligible point/spot (BuildClusterableLightNames from manifest light.cluster) into ClusteredLightContainer (glTF falloff converted) or disable light UBOs. Sets level.punctualLightingMode and level.clusteredLights. Directional suns stay forward for shadows. Scene budget from Blender Export panel → scene.lightBudget / clusterPunctualLights." },
    ],
  },
  {
    id: "cameras",
    ...
    steps: [
      ...
      { file: "packages/engine/src/subsystems/cameras/typed/buildTypedCamera.ts", symbol: "BuildTypedCamera",
  ...
  },
  {
    ...
    steps: [
      ...
      { file: "packages/engine/src/subsystems/shadows/index.ts", symbol: "SetupShadows",
  ...
  },
  {
    ...
    intro: "Scene-wide post stack attached to the active camera after Level.Begin so runtime cameras from OnStart receive the pipeline; OnPostReady runs after attach.",
    steps: [
      { file: "blender_addon/export/post_processing.py", symbol: "serialize_post_processing",
        note: "EXPORT — scene.postProcessing block: MSAA, FXAA, bloom, DOF, SSAO, tone mapping, color grading LUTs (.3dl / .cube / PNG → post/)." },
      { file: "packages/engine/src/core/loader/finalizeLevel.ts", symbol: "FinalizeLevel",
        note: "Order: scene settings → atmosphere → animations → async tasks → constraints → Begin (OnStart may swap camera) → ApplyLateRendering → NotifyPostReady (OnPostReady for zone LUT / pipeline work)." },
      { file: "packages/engine/src/subsystems/postprocess/index.ts", symbol: "ApplyPostProcessing",
        note: "DefaultRenderingPipeline + SSAO2 on scene.activeCamera → level.post. .3dl/PNG via ColorGradingTexture; .cube via CubeColorGradingTexture (postprocess/cubeLutTexture.ts). ApplyColorGradingLut exported for zone behaviors (call from OnPostReady)." },
      { file: "blender_addon/export/component_serializers.py", symbol: "_serialize_vars",
        note: "SCRIPT vars: @exposed({ type: \"file\" }) copies picked assets to post/ (copy_asset) like scene LUT export." },
      { file: "packages/engine/src/subsystems/postprocess/index.ts", symbol: "RetargetPostProcessing",
