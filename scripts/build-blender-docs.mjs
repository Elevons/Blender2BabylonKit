...
export const AREA_PAGES = {
  "index.html": {
    ...
    nodes: [
      ...
      N(4, 40, 300, "scene/", "scene settings", "BJSSceneSettings on Scene.bjs_scene: clear/ambient, environment (World texture via scene/environment.py find_world_env_node, or useDefault flag via use_default_environment), Show Skybox (IBL without visible background when off), Skybox Ignores Fog (mesh.applyFog = false when on), fog, freeze shadows; post-processing lives on the nested bjs_scene.post block (scene/post_processing.py) — MSAA, FXAA, bloom, SSAO, image processing (tone mapping type, exposure, contrast, vignette, color grading, curves), sharpen, DOF, chromatic aberration, grain, glow.", [["On", "Scene.bjs_scene.post"]]),
  ...
  "export.html": {
    ...
    nodes: [
      ...
      N(9, 800, 380, "serialize_scene", "scene block", "Clear/ambient, environment (World Output chain → env/ via scene/environment.py, or useDefault when Default Environment is on; createSkybox; skyboxIgnoreFog), fog, post (export/post_processing.py; LUTs → post/), inputActions + defaultInputMap.", [["File", "export/scene.py"]]),
    ],
    edges: [
