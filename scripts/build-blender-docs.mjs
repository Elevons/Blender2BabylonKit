...
const AREA_PAGES = {
  "index.html": {
    ...
    nodes: [
      ...
      N(2, 40, 150, "properties.py", "component data", "BJSComponent (one group, per-type fields), exposed vars + list items, trigger events + gui3d click events, light/shadow/animation settings. GUI3D_* membership sets (controls/panels/textured). Owns ID_KEY and ensure_object_id().", [["Key class", "BJSComponent"], ["Identity", "ID_KEY = bjs_id"]]),
      ...
      N(6, 520, 190, "export.py", "the heart", "Writes the glb (Blender glTF exporter, +Y-up, GUIDs in extras) and builds the schema-v4 manifest. Converts axes Blender→Babylon. Force-includes referenced objects. Copies audio, GUI JSON, particle JSON, and 3D button images via _copy_asset.", [["Output", "glb + scene.json"], ["Schema", "v4"]]),
      ...
      N(9, 760, 120, "validate.py", "pre-export checks", "Catches silent failures before export: missing scripts, dangling refs, MESH+DYNAMIC, mesh triggers, GUI MESH-on-non-mesh, 3D mesh button on non-mesh, empty 3D panels, missing GUI/particle files and click targets, constraint ends without physics, skinned-mesh components, area lights, duplicate GUIDs, missing audio, no camera, Input Actions.", [["Entry", "validate_scene"]]),
  ...
  "data-model.html": {
    ...
    nodes: [
      ...
      N(2, 40, 170, "BJSComponent", "one group, many faces", "A single PropertyGroup holding fields for every component type, switched by comp_type (TAG/COLLIDER/RIGIDBODY/SCRIPT/CAMERA/AUDIO/CONSTRAINT/GUI/PARTICLE/GUI3D_*). One collection of these lives on every object (obj.bjs_components).", [["Switch", "comp_type enum"], ["On", "Object"]]),
      ...
      N(5, 300, 290, "BJSTriggerEvent", "on-enter / on-click rows", "Trigger collider On Enter reactions AND 3D GUI button On Click reactions: target object + message (+ optional tag filter for triggers). Serialized into collider.events or gui3d events[].", [["Lives in", "comp.trigger_events / gui3d_events"]]),
  ...
  "export.html": {
    ...
    nodes: [
      ...
      N(6, 560, 300, "_serialize_components", "per object", "One dict per component. Converts collider center/size/rotation and constraint pivot/axis to Babylon Y-up; attaches trigger events; copies GUI/particle JSON and 3D button images; serializes GUI3D_* layout and click events.", [["Axis conv", "here"]]),
...
const TRACES = [
  ...
  },
  {
    ...
    steps: [
      { file: "blender_addon/properties.py", symbol: "BJSComponent", note: "AUTHORING — one PropertyGroup, many faces. GUI3D_* types share gui3d_text/image/tooltip/events and panel layout fields. Membership sets GUI3D_CONTROLS / GUI3D_PANELS / GUI3D_TEXTURED classify draw + export branches." },
      { file: "blender_addon/ui.py", symbol: "_draw_click_events", note: "The On Click Events box for 3D controls — same target+message UI as trigger colliders, wired to bjs.gui3d_event_add/remove operators." },
      { file: "blender_addon/export.py", symbol: "_serialize_components", note: "GUI3D_* cases emit type-specific fields; button images copied via _copy_asset to gui/; click-event targets yielded by _iter_referenced_objects for GUID assignment." },
      { file: "blender_addon/validate.py", symbol: "_check_gui3d", note: "Mesh button on non-mesh, panel without button children, click events without targets, missing button image files." },
      { file: "blender_addon/validate.py", symbol: "_check_media", note: "GUI (missing JSON, MESH mode on non-mesh) and PARTICLE (missing JSON) checks run alongside 3D GUI validation." },
    ],
  },
];
