...
export const AREA_PAGES = {
  ...
  "data-model.html": {
    ...
    nodes: [
      ...
      N(2, 40, 170, "BJSComponent", "one group, many faces", "A single PropertyGroup holding fields for every component type, switched by comp_type (TAG/COLLIDER/RIGIDBODY/SCRIPT/CAMERA/AUDIO/CONSTRAINT/GUI/PARTICLE/MSDF_TEXT/GUI3D_*). One collection of these lives on every object (obj.bjs_components).", [["Switch", "comp_type enum"], ["File", "components/component.py"]]),
  ...
  "export.html": {
    ...
    nodes: [
      N(1, 40, 200, "export_level", "orchestrator", "The entry the Export operator and Live Link both call. begin_asset_export → dedupe GUIDs → ensure GUIDs → stamp viewport visibility → write glb → clear visibility stamp → build manifest → attach debug flag → write .scene.json. Returns (glb, json, entity count).", [["File", "export/level.py"]]),
...
export const TRACES = [
  {
    ...
    steps: [
      ...
      { file: "blender_addon/export/level.py", symbol: "export_level", note: "The orchestrator. begin_asset_export (stable side-file paths) → dedupe + ensure GUIDs (before the glb so extras carry them) → stamp viewport visibility → write the glb → clear visibility stamp → build the manifest → attach the Debug Build flag → write .scene.json. Returns (glb, json, count)." },
      { file: "blender_addon/export/level.py", symbol: "_stamp_viewport_visibility", note: "Transient obj['bjs_visible'] = False for renderable objects with hide_viewport (eye icon). Cleared after the glb is written. Render-disabled objects are omitted entirely (_is_renderable / use_renderable=True)." },
