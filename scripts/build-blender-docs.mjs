...
export const AREA_PAGES = {
  "index.html": {
    ...
    nodes: [
      ...
      N(4, 40, 300, "scene/", "scene settings", "BJSSceneSettings on Scene.bjs_scene: clear/ambient color, environment, fog, and post-processing toggles — the scene-wide render config edited in Properties › Scene.", [["On", "Scene.bjs_scene"]]),
      ...
      N(6, 300, 200, "operators/", "the verbs", "Component verbs (add/remove/duplicate/move/copy/cut/paste, Assign GUID, Fit Collider), script pick + Sync, and Validate / Export. Pure behavior — every UI button routes here.", [["Export op", "BJS_OT_export"]]),
      N(7, 300, 340, "ui/", "panels + menus", "All presentation. Viewport 'Babylon' N-panel = the selected object (components + light/camera/animation child panels + quick export). Properties › Scene 'Babylon' = scene-wide (rendering, fog, post, Input Actions, export). Both export blocks share one draw_export_controls().", [["Object", "N-panel"], ["Scene", "Properties › Scene"]]),
  ...
  "data-model.html": {
    ...
    nodes: [
      ...
      N(3, 300, 60, "BJSExposedVar", "script values", "One per @exposed field on a SCRIPT component: name, type, and the value (float/bool/string/vector/color/entity/enum). Object & enum changes fire update callbacks.", [["Lives in", "comp.vars"], ["File", "components/exposed_vars.py"]]),
      N(4, 300, 180, "BJSListItem", "list elements", "Elements of an exposed list field (e.g. entity lists). add_list_item seeds the right value type.", [["Lives in", "var.items"], ["File", "components/exposed_vars.py"]]),
  ...
  "export.html": {
    ...
    nodes: [
      ...
      N(9, 800, 380, "serialize_scene", "scene block", "Color, environment (copied), fog, post, inputActions + defaultInputMap. inputActions is built by input_actions/serialize.py.", [["File", "export/scene.py"]]),
    ],
    edges: [
