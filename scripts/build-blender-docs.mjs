...
export const AREA_PAGES = {
  "index.html": {
    ...
    nodes: [
      ...
      N(3, 40, 180, "components/", "per-object data", "The component model on every Object: BJSComponent (one group switched by comp_type), exposed vars + list items, trigger/click events, light-shadow & animation blocks, and clipboard.py (deep-copy plus PropertyGroup row names for library-override inserts). constants.py isolates every enum table.", [["Key class", "BJSComponent"], ["On", "Object.bjs_components"], ["Types", "incl. MSDF_TEXT"]]),
  ...
  "data-model.html": {
    ...
    nodes: [
      ...
      N(2, 40, 170, "BJSComponent", "one group, many faces", "A single PropertyGroup holding fields for every component type, switched by comp_type (TAG/COLLIDER/RIGIDBODY/SCRIPT/CAMERA/AUDIO/CONSTRAINT/GUI/PARTICLE/MSDF_TEXT/GUI3D_*). One collection of these lives on every object (obj.bjs_components). Registered via core/props.py with LIBRARY_OVERRIDABLE (+ USE_INSERTION on collections) so prefab overrides can edit and insert rows. Each local insert needs a unique PropertyGroup name (not display_name) so Blender INSERT_AFTER ops persist and keep order — clipboard.py assigns those. See PREFABS.html.", [["Switch", "comp_type enum"], ["File", "components/component.py"], ["Prefabs", "PREFABS.html"]]),
